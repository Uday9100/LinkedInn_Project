...
# Parameters for S3
bucket_name = 'udaykafkaproject1'  # Updated S3 bucket name
file_key = 'Linkedin DataSet/jobs/benefits.csv'  # Updated S3 key for your CSV file

# Try fetching CSV file from S3
try:
    response = s3.get_object(Bucket=bucket_name, Key=file_key)
    csv_content = response['Body'].read().decode('utf-8')  # Read and decode CSV
    benefits = pd.read_csv(StringIO(csv_content))  # Convert to DataFrame
    print(benefits.head())  # Print first few rows to verify
...
for index, row in benefits.iterrows():
    ...
    try:
        producer.send('benefits_Producer', value=message.encode('utf-8'))  # Updated topic name
