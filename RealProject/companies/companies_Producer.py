...
file_key = 'Linkedin DataSet/companies/companies.csv'  # Updated S3 key for companies CSV file

# Try fetching CSV file from S3
try:
    response = s3.get_object(Bucket=bucket_name, Key=file_key)
    csv_content = response['Body'].read().decode('utf-8')  # Read and decode CSV
    companies = pd.read_csv(StringIO(csv_content))  # Convert to DataFrame
    print(companies.head())  # Print first few rows to verify
...
for index, row in companies.iterrows():
    ...
    try:
        producer.send('companies_Producer', value=message.encode('utf-8'))  # Updated topic name
